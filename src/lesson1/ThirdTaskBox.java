...
public class ThirdTaskBox <T extends  ThirdTaskFruit> {
    ArrayList<T> fruits;


    public ThirdTaskBox(ArrayList<T> fruits){
        this.fruits = fruits;
    ...
    public boolean compare(ThirdTaskBox<?> fruitBox){
        return Math.abs(this.getWeight() - fruitBox.getWeight()) < 0.001;
    }

    public ArrayList<T> swapFruits(ThirdTaskBox<T> swapBoxes){
        swapBoxes.fruits.addAll(this.fruits);
        this.fruits.clear();
...
/*
    Есть классы Fruit -> Apple, Orange (больше фруктов не надо);
        ...
        Написать метод, который позволяет пересыпать фрукты из текущей коробки в другую (помним про сортировку фруктов: нельзя яблоки
